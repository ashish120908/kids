...
export default function CompareGame() {
  ...
  const startGame = (lvl) => {
    ...
    for (let i = 0; i < TOTAL; i++) {
      const q = generateMathQuestion('compare', lvl);
      const parsed = q.question.match(/(\d+)\s*\?\s*(\d+)/);
      const a = parsed ? parseInt(parsed[1], 10) : 7;
      const b = parsed ? parseInt(parsed[2], 10) : 4;
      generatedList.push({
        id: q.id,
        a,
        b,
      ...
      recordQuestionAnswered('compare', q.id);
    }
  ...
  const handleAnswer = useCallback((choice) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setFeedback(null);
      if (current + 1 >= TOTAL) {
        saveScore('compare', level, score + (correct ? 1 : 0), TOTAL);
        playGameComplete();
        setPhase('done');
      } else {
        setCurrent(c => c + 1);
  ...
  const q = questions[current] || { a: 7, b: 4, answer: '>', choices: ['>', '<', '='] };
