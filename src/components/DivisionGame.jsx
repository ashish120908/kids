...
export default function DivisionGame() {
  ...
  const startGame = (lvl) => {
    ...
    for (let i = 0; i < TOTAL; i++) {
      const q = generateMathQuestion('division', lvl);
      const parsed = q.question.match(/(\d+)\s*÷\s*(\d+)/);
      const a = parsed ? parseInt(parsed[1], 10) : 12;
      ...
      recordQuestionAnswered('division', q.id);
    }
  ...
  const handleAnswer = useCallback((choice) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setFeedback(null);
      if (current + 1 >= TOTAL) {
        saveScore('division', level, score + (correct ? 1 : 0), TOTAL);
        playGameComplete();
        setPhase('done');
      } else {
        setCurrent(c => c + 1);
  ...
  const q = questions[current] || { a: 12, b: 3, answer: 4, choices: [2, 4, 3, 5] };

  return (
    <SpaceGameLayout
      gameTitle="Division Game"
      level={level}
      current={current}
      total={TOTAL}
      score={score}
