...
export default function DivisionGame() {
  ...
  const makeQuestions = useCallback((lvl) => generateRound('division', lvl, TOTAL), []);
  const g = useQuizGame({ gameKey: 'division', total: TOTAL, makeQuestions });

  if (g.phase === 'pick') {
    return <LevelPicker gameName="division" gameTitle="Division Game" gameEmoji="➗" onSelectLevel={g.startGame} />;
  }

  if (g.phase === 'done') {
    return (
      <ScoreSummary
        score={g.score} total={TOTAL} gameName="Division Game" level={g.level}
  ...
  return (
    <SpaceGameLayout
      gameTitle="Division" level={g.level} current={g.current} total={TOTAL} score={g.score}
