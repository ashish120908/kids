...
function generateQuestion(level) {
  ...
  let b = randomInt(1, cfg.maxNum);
  if (a < b) { const tmp = a; a = b; b = tmp; }
  const answer = a - b;
  const choices = generateMultipleChoices(answer, Math.max(0, answer - 10), answer + 10, cfg.choiceCount);
...
export default function SubtractionGame() {
  ...
  const handleAnswer = useCallback((choice) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (current + 1 >= TOTAL) {
        saveScore('subtraction', level, newScore, TOTAL);
        setScore(newScore);
        setPhase('done');
      } else {
        setScore(newScore);
  ...
  return (
    <div className="game-container">
      ...
      <h2 className="game-title">Level {level} — Subtraction Game ➖</h2>
